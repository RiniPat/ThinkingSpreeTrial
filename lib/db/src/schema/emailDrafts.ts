...
export const emailDraftsTable = pgTable("email_drafts", {
  ...
  // Gmail thread id of the sent message. Stored on the PRE email so the POST
  // email can be sent as a reply within the same thread.
  gmailThreadId: text("gmail_thread_id"),
  // The RFC 2822 Message-ID header we set on the sent message. Used to
  // populate In-Reply-To / References on the reply (post) email so Gmail and
  // the founder's client thread them together.
