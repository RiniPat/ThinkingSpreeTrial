...
export function EmailComposer({
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "Email sent",
        description: `Sent to ${toEmail} via Gmail. Timeline updated.`,
      });
  ...
  const canSend = !!subject && !!body && !!toEmail && toEmail.includes("@") && !toEmail.includes("@placeholder.local");
      ...
      <div className="relative w-full max-w-3xl rounded-xl border border-border bg-card shadow-2xl"
        ...
        <div className="max-h-[calc(100vh-16rem)] overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div>
            ...
            {!canSend && toEmail.includes("@placeholder.local") && (
