...
export default function PreSprintPage() {
  ...
  // Selection + active tab persist in the URL query (?company=..&tab=..) so a
  // hard refresh — or coming back after switching browser tabs — restores the
  // exact company and analysis the consultant was on, instead of snapping back
  // to a blank "New company". (The AuthGuard fix keeps the component mounted;
  // this keeps the *right* state even across a full reload or shared link.)
  const initialParams = new URLSearchParams(typeof window !== "undefined" ? window.location.search : "");
  const initialCompany = Number(initialParams.get("company"));
  const initialTab = initialParams.get("tab") as TabKey | null;

  const [selectedId, setSelectedId] = useState<number | null>(
    Number.isFinite(initialCompany) && initialCompany > 0 ? initialCompany : null,
  );
  ...
  useEffect(() => {
    ...
    if (selectedId) params.set("company", String(selectedId)); else params.delete("company");
    if (tab && tab !== "overview") params.set("tab", tab); else params.delete("tab");
